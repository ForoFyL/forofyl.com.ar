...
*/
	...
	if ($sql_where)
	{
		...
		foreach ($rowset as $row)
		{
			...
			if (!$auth->acl_get('m_harddelete', $forum_id) && ((isset($row['post_deleted']) && $row['post_deleted'] != 0 && $row['post_deleted'] != $user->data['user_id']) || (isset($row['topic_deleted']) && $row['topic_deleted'] != 0 && $row['topic_deleted'] != $user->data['user_id'])))
			{
				$total_match_count--;
				continue;
			}
			...
			$template->assign_block_vars('searchresults', array_merge($tpl_ary, array(
				'S_DELETED'			=> ((isset($row['topic_deleted']) && $row['topic_deleted'] != 0) || (isset($row['post_deleted']) && $row['post_deleted'] != 0)) ? true : false,
				'FORUM_ID'			=> $forum_id,
