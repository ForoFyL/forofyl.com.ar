...
soft_delete_update_reply_count($topic_data);
...
get_soft_delete_users($rowset, $user_cache);
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'U_DELETE'			=> ($delete_allowed && ($row['post_deleted'] == 0 || $auth->acl_get('m_harddelete', $forum_id))) ? append_sid("{$phpbb_root_path}posting.$phpEx", "mode=delete&amp;f=$forum_id&amp;p={$row['post_id']}") : '',
